(function(){
    ...
    Date.prototype.Interval = function( interval , tipo) 
    { 
        var times = { d : 1000*60*60*24, h : 1000*60*60 , m : 1000*60 , s : 1000}; 
        var total = ( times[tipo] == undefined ? times.d : times[tipo] ) * interval; 
        var newdt = new Date( this.getTime() + total ); 
        return newdt;
    };
    ...
    this.__ = {
        ...
        GET : function(request, callback)
        {
            ...
            if(__loader.iphoneDevice())
            {
                var cookie_app = webix.storage.local.get("ios_devapp");

                var xhr = webix.ajax();

                xhr.withCredentials = true;

                xhr.get( that.req(request), {}, function(text, xml, xhr)
                {  
        ...
        drop_modal_active: function(array_modal)
        {
            var has_drop = false;

            for(var i in array_modal)
            {
                var item = array_modal[i];

                var is_selector = item.startsWith("[");

                if(is_selector == false)
                {
                    if( $$(item) != undefined )
                    {
                        $$(item).destructor();
                ...
                else
                {
                    ...
                    if(sel != null)
                    {
                        var id = sel.getAttribute("view_id");
        ...
        resize_modal_active: function(item)
        { 
            if( $$(item) != undefined )
            {
                $$(item).resize();
